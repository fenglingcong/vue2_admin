<template>
  <div id="exportSelected">
    ...
    <el-table :data="json" v-loading.body="loading" element-loading-text="拼命加载中..." border style="width: 100%" :header-cell-style="{'text-align': 'center'}" @selection-change="handleSelectionChange">
  ...
  methods: {
    ...
    handleName () {
      if (this.selectedJson.length <= 0) {
        this.$message.warning('请至少选择一项记录')
        return false
      }
    ...
    handleExport () {
      ...
      require.ensure([], () => {
        ...
        const filterJson = this.filterJson(this.selectedJson)
